# Text-to-Speech: error handling (Python, native REST)

The native counterpart to the [SDK recipe](../../sdk/error-handling) — handle the raw
REST error envelope over `requests`. Self-demonstrating: it deliberately calls
...
- Checks `resp.ok` / `resp.status_code`; on failure parses the error envelope
