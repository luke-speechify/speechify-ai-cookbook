# Text-to-Speech: error handling (Python, SDK)

Handle Speechify API errors correctly with the `speechify-api` SDK. Self-demonstrating:
it deliberately calls `client.audio.speech(...)` with a bogus `voice_id` to trigger a
real `404 voice_not_found`, then handles it cleanly.
...
- Catches `ApiError` (from `speechify.core.api_error`) specifically, so real bugs still surface.
- Reads `err.status_code` (HTTP status) and the machine-readable code at
...
- On `429`, honour the `Retry-After` header; the error envelope also carries a `docs_url`.
- Log the `Speechify-Request-Id` response header (a.k.a. `request_id` in the body) on every error.
