# Text-to-Speech: voice language + model support (TypeScript, SDK)
...
- `client.voices.list({ locale: "en", model: "simba-3.2" })` → voices advertising that
  model in that locale. Iterate the page and print each voice's `id`, `display_name`,
...
- `client.voices.get({ voice_id })` → one voice's supported models + languages. A missing
  id throws `SpeechifyError` with `statusCode` 404 (`voice_not_found`) — caught and skipped
  so the recipe still completes.
