# Text-to-Speech: voice language + model support (TypeScript, native REST)

The native counterpart to the [SDK recipe](../../sdk/voice-language-model-support) —
`GET /v1/voices` over raw `fetch`. Answers which model + language combinations a voice
...
- `GET https://api.speechify.ai/v1/voices?locale=en&model=simba-3.2` with
  ...
  `locale`, and its full model→locales map from `voice.models` (each entry has `name` and
